...
const Rnavbar = () => {
  ...
  return (
    <div className='fixed z-[999] w-full px-6 py-4 md:px-20 md:py-8 gap-y-4 md:gap-y-0 flex flex-wrap justify-between items-center bg-zinc-900 sm:bg-transparent'>
      <div className='logo'>
      ...
      <div className={`w-full md:flex md:items-center md:w-auto ${isOpen ? 'block' : 'hidden'}`}>
        <div className='links gap-4 md:gap-10 block md:flex'>
          {["Services","Our Work","About Us","Insights", "Contact"].map((item, index) => (
            <a key={index} className={`text-lg font-semibold capitalize block ${index === 4 && 'ml-0 md:ml-32'}`}>
              {item}
            </a>
          ))}
        </div>
